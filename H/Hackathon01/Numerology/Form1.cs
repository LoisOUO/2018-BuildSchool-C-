...
namespace Numerology
{
    public partial class Form1 : Form
    {
        ...
        static private List<Num> listt2;
        public Form1()
        {
            InitializeComponent();
            list = StarsDay();
        ...
        {   //查詢按鈕
            ...
            string what_date = dateTimePicker1.Value.ToString("yyyy/MM/dd"); //日期
            int Daynumerology1 = 0;
            for (int i = 0; i < what_date.Length; i++)
            {
                if (what_date[i] == '/') //如果有/不做
                { }
                else
                {
                    Daynumerology1 += int.Parse(what_date[i].ToString()); //j=23
                }
            }
            int N = NumNumerology(Daynumerology1);
            int numerology1 = NumNumerology(N);

            

            // System.Diagnostics.Debug.Write("生命靈數:" + numerology1);
            label4.Text = $"生命靈數:{numerology1}"; //=(""生命靈數:"+numerology1)
            // System.Diagnostics.Debug.Write();
            string WhatMonth = dateTimePicker1.Value.ToString("MM"); //找生日的月份
            string Whatday = dateTimePicker1.Value.ToString("dd");    //找生日的日期

            foreach (var item in list)
            {
                //var S = item.star+item.monthStars+item.dayStars+item.monthEnd+item.dayEnd;
                //  System.Diagnostics.Debug.WriteLine(MM.ToString());
                if ((WhatMonth == item.monthEnd && int.Parse(Whatday) <= item.dayEnd) || (WhatMonth == item.monthStars && int.Parse(Whatday) >= item.dayStars))
                {
                    System.Diagnostics.Debug.WriteLine(item.star);

                    label5.Text = item.star;
                }
            }
            label6.Text = inputStars(label5.Text,numerology1);
            
          //  System.Diagnostics.Debug.WriteLine();
          
        }
        //int NumNumerology(int a) //判斷字串是否大於兩個數字 如果大於就相加=b
        int NumNumerology(int a) //判斷字串是否大於兩個數字 如果大於就相加=b
        {
            int input = a;
            string A = a.ToString();
        ...
        string inputStars(string b,int c) //b是星座名稱
        {
        ...
        static List<MyData> StarsDay()
        {
            var list = new List<MyData>();
            list.Add(new MyData { star = "水瓶座", monthStars = "01", dayStars = 21, monthEnd = "02", dayEnd = 20 }); //  水瓶座
            list.Add(new MyData { star = "雙魚座", monthStars = "02", dayStars = 21, monthEnd = "03", dayEnd = 20 }); //雙魚座
            list.Add(new MyData { star = "牡羊座", monthStars = "03", dayStars = 21, monthEnd = "04", dayEnd = 20 }); //牡羊
            list.Add(new MyData { star = "金牛座", monthStars = "04", dayStars = 21, monthEnd = "05", dayEnd = 20 }); //金牛
            list.Add(new MyData { star = "雙子座", monthStars = "05", dayStars = 21, monthEnd = "06", dayEnd = 20 }); //雙子
            list.Add(new MyData { star = "巨蟹座", monthStars = "06", dayStars = 21, monthEnd = "07", dayEnd = 20 }); //巨蟹
            list.Add(new MyData { star = "獅子座", monthStars = "07", dayStars = 21, monthEnd = "08", dayEnd = 20 });    //獅子
            list.Add(new MyData { star = "處女座", monthStars = "08", dayStars = 21, monthEnd = "09", dayEnd = 20 });  //處女
            list.Add(new MyData { star = "天秤座", monthStars = "09", dayStars = 21, monthEnd = "10", dayEnd = 20 }); //天秤
            list.Add(new MyData { star = "天蠍座", monthStars = "10", dayStars = 21, monthEnd = "11", dayEnd = 20 }); //天蠍
            list.Add(new MyData { star = "射手座", monthStars = "11", dayStars = 21, monthEnd = "12", dayEnd = 20 }); //射手
            list.Add(new MyData { star = "魔羯座", monthStars = "12", dayStars = 21, monthEnd = "01", dayEnd = 20 }); //魔羯
